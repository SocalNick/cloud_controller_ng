...
RSpec.describe AppBitsPackage do
  ...
  describe '#create_package_in_blobstore' do
    ...
    it 'sets the package sha to the package' do
      expect { packer.create_package_in_blobstore(package_guid, uploaded_package_zip, cached_files_fingerprints) }.to change { package.refresh.package_hash }
    end

    it 'sets the state of the package' do
      expect { packer.create_package_in_blobstore(package_guid, uploaded_package_zip, cached_files_fingerprints) }.to change { package.refresh.state }.to('READY')
    end
    ...
    context 'when the app bits are too large' do
      ...
      it 'raises an exception and deletes the bits' do
        expect(FileUtils).to receive(:rm_f).with(uploaded_package_zip)
        expect { packer.create_package_in_blobstore(package_guid, uploaded_package_zip, cached_files_fingerprints) }.to raise_error(CloudController::Errors::ApiError, /may not be larger than/)
      end
